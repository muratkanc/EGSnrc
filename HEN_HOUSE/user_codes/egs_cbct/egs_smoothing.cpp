...
EGS_Distribution2DArray::EGS_Distribution2DArray(const char *fname,
    ...
    nz=Nz;
    int nreg = Nx*Ny;
    float* d = new float [nreg];
    float* err = new float [nreg];
...
void EGS_Smoothing::setNmax(int Nmax) {
    ...
    nmax = Nmax;
    cc1 = new double [nmax+1];
    cc2 = new double [nmax+1];
    cc3 = new double [nmax+1];
    cc4 = new double [nmax+1];
    cc5 = new double [nmax+1];
    cc6 = new double [nmax+1];
    ccc = new double [nmax+1];
    for(int n=2; n<=nmax; n++) {
        double d0 = 2*n+1;
        double d2 = d0/3*n*(n+1);
        double d4 = 0.2*d2*(3*n*n+3*n-1);
        double d6 = d2/7*(3*n*n*n*n+6*n*n*n-3*n+1);
        double del1 = d0*d4-d2*d2;
        double del2 = d2*d6-d4*d4;
        cc1[n]=d4/del1;
        cc2[n]=d2/del1;
        cc3[n]=d0/del1;
        cc4[n]=d6/del2;
...
bool EGS_Smoothing::smooth1D(int iii, int Nx, int Ni, int &Nmax,
    ...
    while(1) {
        ...
        pii = (sum2/N-p0)/bi;
        p0 -= ai*pii;
        pj = sum1*cc4[Nmax]-sum3*cc5[Nmax];
        pjj = sum3*cc6[Nmax]-sum1*cc5[Nmax];
        double chi2 = 0, chi2a = 0;
        int nc = 0;
        se2 = 0;
        double se2a = 1e30;
        for(register int i=-Nmax; i<=Nmax; i++) {
            ...
            if( df > 0 ) {
                ++nc;
                register double fs = p0 + pi*i + pii*i*i - d_array[ireg];
        ...
        if( nc < 4 ) {
            Nmax = 1;    // minimum possible window np+1
            return false;
        }
    ...
    for(register int j=-nj; j<=nj; j++) {
        ...
        for(register int i=-ni; i<=ni; i++) {
            ...
            if( df > 0 ) {
                ...
                chi2 += fs*fs/df;
                register double aux2 = aux1 + (ai-i*i)/bi;
                se2 += aux2*aux2*df;
            }
        }
    }
...
EGS_Smoothing::smooth1(EGS_Distribution2D *the_dose) {
    ...
    fsmoo = result->d_array;
    dfsmoo = result->error_array;
    d_array = the_dose->d_array;
    error_array = the_dose->error_array;
    dmax = 0;
    double sume2 = 0;
    bool *smoothed = new bool [nreg];
    fsmoo1 = the_dose->d_array;
    dfsmoo1 = the_dose->error_array;
    for(ii=0; ii<nreg; ii++) {
        if( d_array[ii] > dmax ) dmax = d_array[ii];
        sume2 += error_array[ii];
        smoothed[ii] = false;
    ...
    //------------------------------------------------------
    ni_array = new char [nreg];
    nj_array = new char [nreg];
    ...
    for(reg=0; reg<nreg; reg++) {
        fsmoo1[reg] = fsmoo[reg];
        dfsmoo1[reg] = dfsmoo[reg];
    }
    for(ii=0; ii<nx; ii++) {
        //egsWarning(".");
        for(jj=0; jj<ny; jj++) {
...
EGS_Smoothing::EGS_Smoothing(EGS_Input *I) {
    ...
    int nm, err;
    EGS_Float tmp;
    err = input->getInput("maximum 2D window",nm);
    if( !err ) nmax3d = nm;
    err = input->getInput("maximum 1D window",nm);
    if( !err ) setNmax(nm);
    err = input->getInput("maximum chi2",tmp);
    if( !err ) chi2_max = tmp;
    err = input->getInput("minimum dose",tmp);
    if( !err ) dmin = tmp;
...
bool EGS_Smoothing3D::smooth_1d(float *darray, int iii, int Nx, int Ni,
    ...
    while(1) {
        ...
        p0 = sum0*Nii;
        pi = sum1*Nii*aii;
        pii = (sum2*aii*Nii-p0)*bi;
        double al = p0;
        p0 -= ai*pii;
        double bl = 6*sum1/(N*N*Nmax);
        double chi2 = 0, chi2l = 0, se2l = 0;
        int nc = 0;
        se2 = 0;
        for(register int i=-Nmax; i<=Nmax; i++) {
            register int ireg = reg + i*Ni;
            ...
            if( df > 0 ) {
                ++nc;
                register double fs = p0 + pi*i + pii*i*i - darray[ireg];
                chi2 += fs*fs/df;
                register double aux1 = 1 + (ai-i*i)*bi;
                se2 += aux1*aux1*df;
...
void EGS_Smoothing3D::set_coeffs() {
    ...
    pjk = sjk*aji*aki*Ni;
    p0 = s0*Ni;
    pii = (sii*aii*Ni-p0)*bii;
    pjj = (sjj*aji*Ni-p0)*bji;
        ...
        for(register int j=-nj; j<=nj; j++) {
            ...
            for(register int i=-ni; i<=ni; i++) {
                ...
                if( df > 0 ) {
                    ...
                    chi2 += fs*fs/df;
                    register double aux2 = aux1 + (ai-i*i)*bii;
                    se2 += aux2*aux2*df;
                }
            }
        }
...
EGS_Distribution3D *EGS_Smoothing3D::smooth1(const EGS_Distribution3D *the_dose) {
    ...
    double sume2 = 0;
    bool *smoothed = new bool [nreg];
    fsmoo1 = new float [nreg];
    dfsmoo1 = new float [nreg];
    for(ii=0; ii<nreg; ii++) {
        if( d_array[ii] > dmax ) dmax = d_array[ii];
        sume2 += error_array[ii];
        smoothed[ii] = false;
        fsmoo1[ii] = d_array[ii];
        dfsmoo1[ii] = error_array[ii];
    }
...
EGS_Smoothing3D::EGS_Smoothing3D(EGS_Input *I) {
    ...
    int nm, err;
    EGS_Float tmp;
    err = input->getInput("maximum 3D window",nm);
    if( !err ) nmax3d = nm;
    err = input->getInput("maximum 1D window",nm);
    if( !err ) set_nmax(nm);
    err = input->getInput("maximum chi2",tmp);
    if( !err ) chi2_max = tmp;
    err = input->getInput("minimum fraction",tmp);
    if( !err ) dmin = tmp;
