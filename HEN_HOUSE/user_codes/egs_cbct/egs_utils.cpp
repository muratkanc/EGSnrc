...
bool EGS_BaseFile::fileExist(const char *pName) {
    fstream in;
    in.open(pName,ios::in | ios::binary);
    if(in.is_open()) {
        in.close();
        return true;
    }
...
void EGS_BinaryFile::writeBinary(const EGS_I64 &pos) {
    ...
    char *cptr = new char[size*sizeof(char)];
    cptr = block;
                                 ...
                                 const EGS_I64 &pos)
{
    ...
    char *cptr = new char[s*sizeof(char)];
    cptr = what;
