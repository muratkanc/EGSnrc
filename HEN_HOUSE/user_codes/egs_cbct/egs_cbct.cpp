    ...
    defined(false), swap(false)
{
    ...
    if (setup) {
        ...
        int error = 0;
        error = setup->getInput("minimum angle",ang);
        if (!error) {
            amin=ang;
        }
        error = 0;
        error = setup->getInput("maximum angle",ang);
        if (!error) {
            amax=ang;
        }
        error = 0;
        error = setup->getInput("orbit",orb);
        if (!error) {
            orbit=orb;
        }
        error = 0;
        error = setup->getInput("step",stp);
        if (!error) {
            step=stp;
        }
        //************************************************
        ...
        error = 0;
        error = setup->getInput("translation",trans);
        if (error) {
        ...
        //**********************************************************
        error = 0;
        error = setup->getInput("projection",ipr);
        if (!error && ipr >= 0) {
            angle = amin + iproj*step;
        }
        else {
            error = 0;
            error = setup->getInput("angle",ang);
            if (!error) {
                angle=ang;
        ...
        if (iproj < 0) {
            error = 0;
            error = setup->getInput("x-rotation",ang);
            if (!error) {
                angle=ang;
                rm = EGS_RotationMatrix(angle*convert,0,0);
                irot=0;
            }
            else {
                error = 0;
                error = setup->getInput("y-rotation",ang);
                if (!error) {
                    angle=ang;
                    rm = EGS_RotationMatrix(0,angle*convert,0);
                    irot=1;
                }
                else {
                    error = 0;
                    error = setup->getInput("z-rotation",ang);
                    if (!error) {
                        angle=ang;
                        rm = EGS_RotationMatrix(0,0,angle*convert);
                        irot=2;
                    }
                    else {
                        error = 0;
                        error = setup->getInput("any-rotation",ang);
                        if (!error) {
                            angle=ang;
                            rm = EGS_RotationMatrix(0,0,angle*convert);
                            irot=2;
        ...
        allowed_types.push_back("yes");
        allowed_types.push_back("no");
...
void EGS_CBCT::fastCompton() {
    ...
    EGS_Vector x(the_stack->x[np],the_stack->y[np],the_stack->z[np]);
    EGS_Float sinpsi = ui.x*ui.x+ui.y*ui.y;
    EGS_Float sindel,cosdel;
    if( sinpsi > 1e-20 ) {
        sinpsi = sqrt(sinpsi);
        sindel = ui.y/sinpsi;
        cosdel = ui.x/sinpsi;
    ...
    EGS_Float bro = 1./broi;
    EGS_Float bro1 = 1 - bro;
    ...
    for(int j=0; j<nsplit; j++) {
        ...
        */
        ...
        if( Ej > the_bounds->pcut ) {
            ...
            EGS_Vector u;
            if( sinpsi > 1e-10 ) {
                EGS_Float us = sint*cphi;
                EGS_Float vs = sint*sphi;
...
int EGS_CBCT::initScoring() {
    ...
    if( options ) {
        ...
        EGS_RotationMatrix rm = EGS_RotationMatrix();
        if (ang>=0) {
            rm = EGS_RotationMatrix(0,ang);   // x-rotation
        }
        else {
            ang*=-1;    // z-rotation
            rm = EGS_RotationMatrix(ang,0);
        }
        ...
        if( type == planar ) {
            ...
            if( !err0 ) {
                ...
                err0 = 0;
                err0 = aux->getInput("minimum Kscat fraction",cut);
                if( !err0 ) {
            ...
            if( !err0 && (eest.size() == 1 || eest.size() == 5) ) {
                ...
                if( error_estimation == 1 && eest.size() == 5 ) {
                    ee_xmin = eest[1];
                    if( ee_xmin < 0 ) ee_xmin = 0;
                    ee_xmax = eest[2];
                    if( ee_xmax > Nx-1 ) ee_xmax = Nx-1;
                    ee_ymin = eest[3];
                    if( ee_ymin < 0 ) ee_ymin = 0;
                    ee_ymax = eest[4];
                    if( ee_ymax > Ny-1 ) ee_ymax = Ny-1;
                }
            }
            ...
            uy = EGS_Vector(-1,0,0);
            ux = EGS_Vector(0,1,0); // if positive z-axis as default
...
// get Emax and Emin
    ...
    int nbin = 0, n_bin;
    for (int l =0; l<geometry->nMedia(); l++) {
        e_max = i_gmfp[l].getXmax();
        e_min = i_gmfp[l].getXmin();
        emax = e_max > emax ? e_max:emax;
        emin = e_min < emin ? e_min:emin;
...
// get media weights
    ...
    EGS_Float *mWeight = new EGS_Float[geometry->nMedia()];
    EGS_Float vacWeight = 0;
    ...
    EGS_Float *xmu = new EGS_Float [nbin];
    EGS_Float *fmu = new EGS_Float [nbin];
...
void EGS_CBCT::initOutput() {
    ...
    if( out ) {
        ...
        if (xy) {
            profile = new EGS_XYProfile(xy,ax,ay,Nx,Ny);
            delete xy;
        }
        ...
        if(scano) {
            ...
            */
            string the_scan;
            out_err = 0;
            out_err = scano->getInput("scan file",the_scan);
            if( out_err ) {
                egsWarning(
...
void EGS_CBCT::initVRT() {
    ...
    if( vr ) {
        ...
        //************************************************
        vector<string> score;
        score.push_back("track_length");
        score.push_back("forced_detection");
        forced_detection = vr->getInput("scoring type",score,1);
        //************************************************
        //     Splitting scheme input
        //************************************************
        vector<int> split;
        vector<EGS_Float> ifsplit;
        ...
        if (split_type==PDIS) {
            ...
            if( attp ) {
                ...
                patt_point = EGS_Vector(0,0,0);
                patt_a = EGS_Vector(0,0,1);
                t->transform(patt_point);
                t->rotate(patt_a);
                cbctS->transform(patt_point);
                cbctS->transform(patt_a);
        ...
        if( !vr->getInput("measured scan",mscan) &&
            ...
            if( msc && mbl ) {
                m_real = new float [Nx*Ny];
                m_blank = new float [Nx*Ny];
        ...
        if( !vr->getInput("splitting geometry",sgeom) ) {
            ...
            if( !split_geom ) egsWarning("\n\n********** no geometry named"
                ...
                if( !vr->getInput("splitting area",sarea) && sarea.size() == 4 ) {
                    split_xmin = sarea[0];
                    if( split_xmin < 0 ) split_xmin = 0;
                    split_xmax = sarea[1];
                    if( split_xmax <= split_xmin )
                        split_xmax = Nx-1;
                    split_ymin = sarea[2];
                    if( split_ymin < 0 ) split_ymin = 0;
                    split_ymax = sarea[3];
                    if( split_ymax <= split_ymin )
                        split_ymax = Ny-1;
                }
                else {
                    split_xmin = 0;
...
// egsInformation("ireg = %d nsec = %d pixel=%d\n",ir0,nsec,k);
    if (!nsec) {
        scoreDirectKerma( gle, p.wt, t, a*p.u, k );
        return;
    }
    /*if (nsec)
...
EGS_Float EGS_CBCT::getAveScatErrorSqr() {
    ...
    EGS_Float sum = 0, scan, dscan, scat, dscat;
    int n=0;
    for (EGS_I32 i = 0; i<Nx*Ny; i++) {
...
void EGS_CBCT::errorDistribution(const char * label, EGS_ScoringArray &kerma )
{
    string suffix = label;
    int hsize = 100;
    vector<int> hist(hsize);
    double min = 0.0, max=-1e30;
...
float * EGS_CBCT::getAttTotalRatio() {
    ...
    for(int i=0; i<Nx; i++) {
        for(int j=0; j<Ny; j++) {
            ...
            if ( cbctS->swapX2Y() ) {
                tot[j+i*Ny] = r > 0 ? float(r1/r) : 0;
            }
            else                   {
                tot[i+j*Nx] = r > 0 ? float(r1/r) : 0;
            }
        }
    }
    return tot;
        ...
        if (real_scan.length() && (scan_type == scatter || scan_type == all)) {
            ...
            for(int i=0; i<Nx; i++) {
                for(int j=0; j<Ny; j++) {
                    ...
                    if ( cbctS->swapX2Y() ) {
                        scan[j+i*Ny] = r*pnorm*norm;
                        err[j+i*Ny] = dr*pnorm*norm;
                    }
                    else                  {
                        scan[i+j*Nx] = r*pnorm*norm;
                        err[i+j*Nx] = dr*pnorm*norm;
                    }
                    if( m_real && m_blank ) {
...
int EGS_CBCT::shower() {
    ...
    if( p.q && p.wt ) {
        egsWarning("This application is not meant to transport "
...
bool EGS_CBCT::computePhotonScore(EGS_CBCT_Photon &p, int nspl,
    ...
    bool not_interacted = false;
    EGS_Float eta, d_eta = 1, wt = p.wt;
    int ispl = 0;
    if( nspl > 0 ) {
        eta = rndm->getUniform();
        if( nspl > 1 ) {
...
do_interaction:
    ...
    EGS_Float wt = p.wt;
    EGS_Float wnew = wt/aspl_tot;

    bool have_scattered_photon = false;
    ...
    if( w_screen < 1 ) {
        ...
        for(int j=0; j<nsample; j++) {
            ...
            */
            EGS_Vector xscreen;
            int isc = pselector->getPoint(rndm,xscreen);
        ...
        for(int j=0; j<nspl; j++) {
            ...
            if( p.E*br > the_bounds->pcut ) {
                ...
                EGS_Float cphi, sphi;
                rndm->getAzimuth(cphi,sphi);
                EGS_Float cost = 1 - temp;
                sint = sqrt(sint);
                p1.u.rotate(cost,sint,cphi,sphi);
                int isc = hitsScreen(p1.x,p1.u,0);
                bool result;
                if( isc >= 0 ) {
                    int ns = keep != j ? 0 : 1;
                    double sc;
    ...
    if( sreg >= 0 ) {
        split_collect_tot += aspl_tot;
        ++split_count_tot;
        if( nc > 0 ) {
            double av = aspl_tot*sctot/nc;
            split_collect[sreg] += av;
            ++split_count[sreg];
        }
    }
...
void EGS_CBCT::selectPhotonMFP(EGS_Float &dpmfp) {
    ...
    */
    ...
    if( mfptr_do && isc >= 0 ) {
        eta = 1/(1-rndm->getUniform());
        EGS_Float aux = sqrt(eta);
...
//                   j,gsections[j].imed,j,gsections[j].t,tstep,this_lambda);
        ...
        if( Lambda < 80 ) {
            ...
            if( !the_stack->latch[np] ) {
                patt_score_sum += sc*aup;
                ++patt_score_count; //<== original: <Ka>
...
void EGS_CBCT::fastTransport(EGS_CBCT_Photon &p) {
    ...
    // *** number of MFP
    //
    EGS_Float lambda, wtr = 1;
    bool check_interaction;
    if( !p.latch && p.isc >= 0 ) {
        check_interaction = true;
        if( mfptr_do ) {
            EGS_Float eta = 1/(1-rndm->getUniform());
            EGS_Float aux = sqrt(eta);
            lambda = mfptr_lamo*(aux-1);
