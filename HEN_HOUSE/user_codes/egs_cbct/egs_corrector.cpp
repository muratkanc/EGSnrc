    ...
    fsplit(fsp), KaPrim(1), Cx(1), NTot(0)
{
    ...
    err0 = 0;
    EGS_Float x_min = Xmin, x_max = Xmax;
    int _step = 1;
    err0 = inp->getInput("minimum correction",x_min);
    if( !err0 ) Xmin = x_min;
    err0 = 0;
    err0 = inp->getInput("maximum correction",x_max);
    if( !err0 ) Xmax = x_max;
    err0 = 0;
    err0 = inp->getInput("update step",_step);
    if( !err0 ) step = _step;
    int Vmax = vx, _Vmax;
    err0 = 0;
    err0 = inp->getInput("maximum resolution",_Vmax);
    if( !err0 ) Vmax = _Vmax;
