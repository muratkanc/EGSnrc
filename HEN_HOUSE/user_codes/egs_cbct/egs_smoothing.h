...
struct EGS_Distribution2DArray {
    ...
    EGS_Distribution2D* get_proj(int i) {
        return scan[i];
    }
...
struct EGS_Distribution3D {
    ...
    void add_dose1(const EGS_Distribution3D *dose) {
        if( nreg != dose->nreg ) return;
        ncase += dose->ncase;
        register double w2 = dose->ncase;
        w2 /= ncase;
        register double w1 = 1 - w2;
        register double w12 = w1*w1;
...
public:
    ...
    void  set_nmax3d(int Nmax) {
        nmax3d = Nmax;
    };
    void  set_nmax(int Nmax);
    void  set_chi2_max(double chi2) {
        chi2_max = chi2;
    };
    void  set_dmin(double Dmin) {
        dmin = Dmin;
    };
