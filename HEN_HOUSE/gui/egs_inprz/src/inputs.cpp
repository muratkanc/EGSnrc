...
{
    in >> rZ->mTitle;
    rZ->errors  = rZ->mTitle->errors;
    in >> rZ->mIO;
    rZ->errors += rZ->mIO->errors;
    in >> rZ->mMC;
    rZ->errors += rZ->mMC->errors;
    if ( print_phd( rZ ) ) {
        in >> rZ->mPHD;
        rZ->errors += rZ->mPHD->errors;
    }
    in >> rZ->mGEO;
    rZ->errors += rZ->mGEO->errors;
    rZ->SetInputMethod( rZ->GetInputMethod() );
    if ( cavity_inp( rZ ) )
        in >> rZ->mCAV;
    rZ->errors += rZ->mCAV->errors;
    in >> rZ->mSRC;
    rZ->errors += rZ->mSRC->errors;
    in >> rZ->mMCP;
    rZ->errors += rZ->mMCP->errors;
    in >> rZ->mPGLS;
    rZ->errors += rZ->mPGLS->errors;
    in >> rZ->mVAR;
    rZ->errors += rZ->mVAR->errors;
    if ( plot_on( rZ ) ) {
        in >> rZ->mPLOT;
        rZ->errors += rZ->mPLOT->errors;
    }
    return in;
}
