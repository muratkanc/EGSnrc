...
MInputRZ* inputRZImpl::GetInputRZ()
{
    ...
    finp->mMC   = GetMC();
    finp->mGEO  = GetGEO();
    geoErrors=finp->mGEO->errors;
    formErrors+=geoErrors;
    finp->mPHD  = GetPHD();
    finp->mCAV  = GetCAV();
    finp->mSRC  = GetSRC();
    finp->mMCP  = GetMCP();
...
//---------------------------------------
    ...
    if ( current_description_by.toLower() == "regions" ) {
        tmpStart = geo->start_reg;
        tmpStop = geo->stop_reg;
        geo->start_reg.clear();
        geo->stop_reg.clear();
    }
    else {
        tmpStart = geo->start_Z;
        tmpStop = geo->stop_Z;
        tmpStartCyl = geo->start_ring;
        tmpStopCyl = geo->stop_ring;
        geo->start_Z.clear();
        geo->stop_Z.clear();
