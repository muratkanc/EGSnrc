...
void MGEOInputs::mapRegions() {
    ...
    if ( description_by.toLower() == "regions") {
        if(start_Z.size()) start_Z.clear();
        if(stop_Z.size()) stop_Z.clear();
        if(start_ring.size()) start_ring.clear();
        if(stop_ring.size()) stop_ring.clear();
        for (unsigned int j = 0; j < start_reg.size(); j++) {
            start_Ztmp.push_back(getZ(start_reg[j]));
            start_Rtmp.push_back(getR(start_reg[j]));
            stop_Ztmp.push_back(getZ(stop_reg[j]));
            stop_Rtmp.push_back(getR(stop_reg[j]));
            if (stop_Rtmp[j] > start_Rtmp[j]) {
                ...
                if (start_Ztmp[j]>1) {
                    ...
                    stop_Z.push_back(nslabs());
                    start_ring.push_back(start_Rtmp[j]);
                    stop_ring.push_back(start_Rtmp[j]);
                    // New regions needed
                ...
                if (stop_Ztmp[j]<nslabs()) {
                    ...
                    start_ring.push_back(stop_Rtmp[j]);
                ...
                start_Z.push_back(1);
                stop_Z.push_back(nslabs());
                start_ring.push_back(start_Rtmp[j]+shift_i);
                stop_ring.push_back(stop_Rtmp[j]-shift_f);
                // New regions needed
        ...
        start_reg = start_IRtmp;
        stop_reg = stop_IRtmp;
    }
    else {
        if (start_reg.size()) start_reg.clear();
        if(stop_reg.size()) stop_reg.clear();
...
std::ifstream & operator >> ( std::ifstream & in, MGEOInputs*  rGEO )
{
    ...
    if (rGEO->inp_meth.toLower() != "cavity information") {
        ...
        rGEO->start_reg.clear();
        rGEO->start_reg.push_back( 0 );
        rGEO->stop_reg.clear();
        rGEO->stop_reg.push_back( 0 );
        ...
        if ( rGEO->description_by.toLower() == "regions") {
            ...
            if ( ZeroRegionPresent ) {// there shouldn't be a zero region
                ...
                rGEO->start_reg.clear();
                rGEO->start_reg.push_back( 2 );
                rGEO->stop_reg.clear();
                rGEO->stop_reg.push_back( n_regions );
                rGEO->mednum.clear();
                rGEO->mednum.push_back( 1 );
            }
            // ***********************************************************************************
        }
        else {
