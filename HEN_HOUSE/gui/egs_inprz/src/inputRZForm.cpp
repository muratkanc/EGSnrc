...
//                     isTclTkInstalled();
    ...
    QStringList smed;
    smed << "medium" << "start region" << "stop region";
    v_float frmed;
    frmed.push_back(0.5);
    frmed.push_back(0.25);
    frmed.push_back(0.25);
    ...
    QStringList s;
    s.append( "# slabs" );
    s.append("thickness [cm]");
    fra[0]=0.3;
    fra[1]=0.70;//fra[2]=0.35;
...
void inputRZImpl::update_MCTParam( const MMCPInputs*  EGSmcp )
{
    ...
    if (EGSmcp->RAY.toLower()=="custom") {
        v_string vstr[2];
        vstr[0]=EGSmcp->ff_media;
        vstr[1]=EGSmcp->ff_files;
        //qt3to4 -- BW
...
void inputRZImpl::fill_media_table( const MGEOInputs* EGSgeo )
{
    ...
    v_int vr[3];
    vr[1] = EGSgeo->start_reg;
    vr[2] = EGSgeo->stop_reg;
    v_int vs[5];
    vs[1] = EGSgeo->start_Z;
    vs[2] = EGSgeo->stop_Z;
    vs[3] = EGSgeo->start_ring;
    vs[4] = EGSgeo->stop_ring;
...
void inputRZImpl::show_about()
{
    ...
    aboutText.replace("XXX",qt_version);
    aboutText.replace("YYY",tr(VERSION));//defined in datainp.h
    QString nrcText = QString(ABOUT_NRC);
