...
public:
          ...
          copyUCs(true), all_defaults_exist(true)
    {
        ...
        canonical = QString(CANONICAL) + arch;

        // configuration file: if exists, defines config and HEN_HOUSE
        QGroupBox *gb = new QGroupBox("configuration group box",this);
        QHBoxLayout *gbl = new QHBoxLayout(gb);
        gbl->setSpacing(6); //gbl->setMargin(11);
        gb->setTitle( tr("Configuration name") );
        confLineEdit = new QLineEdit(gb);
        QString conf_temp = egsConfiguration();
        ...
        gbl = new QHBoxLayout(gb);
        gbl->setSpacing(6); //gbl->setMargin(11);
        gb->setTitle( tr("System location (a.k.a. HEN_HOUSE)") );
        henLineEdit = new QLineEdit("HEN_HOUSE line edit",gb);
        QString hen_temp = henHouse();
        ...
        typical = new QRadioButton("&Typical");
        custom = new QRadioButton("&Custom");
        gbl->addWidget(typical);
        gbl->addWidget(custom);
    ...
    void    setConfName(const QString & cn) {
        conf_name = cn;
    }
    QString getCanonical() {
        return canonical;
    }
    void    setCanonical(const QString & can) {
        canonical = can;
    }
    void setNeedsUCs(bool need) {
        copyUCs=need;
    }
    bool needsUCs() {
        return copyUCs;
...
public slots:
...
#else
        else if (MCompiler(C,"c89").exists()) {
            MCompiler c(C,"c89");
            *cc = c;
            return true;
        }
        else if (MCompiler(C,"c90").exists()) {
            MCompiler c(C,"c90");
            *cc = c;
            return true;
        }
        else if (MCompiler(C,"c99").exists()) {
            MCompiler c(C,"c99");
            *cc = c;
            return true;
        }
    ...
    void setConfigField(const QString &conf) {
        QString the_conf = conf;
        the_conf.truncate(the_conf.lastIndexOf("."));
