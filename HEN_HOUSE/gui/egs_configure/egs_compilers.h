...
public:
        ...
        : QDialog(parent), m_Compiler(a_Compiler)
    {
        ...
        optEdit->setText(m_Compiler->optimization());
        optEdit->setCursorPosition(0);
        ...
        versionEdit->setText(m_Compiler->version());
        versionEdit->setCursorPosition(0);
...
public:
        ...
        : QWizardPage(parent), settings(0), fc(a_f), cc(a_c), cpp(a_cpp), make(a_m)
    {
        ...
        QPushButton *b = new QPushButton(tr("..."),gb);
        ...
        fcCB = new QComboBox(gb);
        fcCB->setSizePolicy(QSizePolicy::Minimum,QSizePolicy::Preferred);
        gbl->addWidget(fcCB);
        ...
        ccCB = new QComboBox(gb);
        ccCB->setSizePolicy(QSizePolicy::Minimum,QSizePolicy::Preferred);
        gbl->addWidget(ccCB);
        b = new QPushButton("...",gb);
        b->setSizePolicy(QSizePolicy::Maximum,QSizePolicy::Preferred);
        ...
        gbl = new QHBoxLayout;
        gb->setTitle( tr("C++ compiler") );
        cppCB = new QComboBox(gb);
        cppCB->setSizePolicy(QSizePolicy::Minimum,QSizePolicy::Preferred);
        gbl->addWidget(cppCB);
        b = new QPushButton("...",gb);
        b->setSizePolicy(QSizePolicy::Maximum,QSizePolicy::Preferred);
...
public slots:
    ...
    */
    ...
    void dsoOptions() {
        ...
        QLineEdit *definesEdit  = new QLineEdit;
        definesEdit->setText(cpp->defines());
        dsoLayout->addRow(tr("Preprocessor &defines:"), definesEdit);

        QLineEdit *fpicEdit  = new QLineEdit;
        fpicEdit->setText(cpp->fpic());
        dsoLayout->addRow(tr("Position independent code (PIC) flag:"), fpicEdit);

        QLineEdit *sharedEdit  = new QLineEdit;
        sharedEdit->setText(cpp->shared());
        dsoLayout->addRow(tr("Shared library flag:"), sharedEdit);

        QLineEdit *dlopenEdit  = new QLineEdit;
        dlopenEdit->setText(cpp->extra());
        dsoLayout->addRow(tr("Output:"), dlopenEdit);
        QLineEdit *dsoPathEdit  = new QLineEdit;
        dsoPathEdit->setText(cpp->dsoPath());
        ...
        QLineEdit *flibsEdit  = new QLineEdit;
        flibsEdit->setText(cpp->flibs());
        dsoLayout->addRow(tr("Fortran libraries needed by C++ linker:"), flibsEdit);

        definesEdit->setCursorPosition(0);
        fpicEdit->setCursorPosition(0);
        sharedEdit->setCursorPosition(0);
        dlopenEdit->setCursorPosition(0);
        dsoPathEdit->setCursorPosition(0);
        linkingEdit->setCursorPosition(0);
        flibsEdit->setCursorPosition(0);

#if defined(Q_OS_MAC) || defined(Q_OS_DARWIN)
        QLineEdit *bundleEdit  = new QLineEdit;
        bundleEdit->setText(cpp->bundleOSX());
        dsoLayout->addRow(tr("Creating library bundles in OSX:"), bundleEdit);
        bundleEdit->setCursorPosition(0);
#endif

        vl->addLayout(dsoLayout);
    ...
    QString getVar(const QString & var) {
        return QString(getenv(var.toLatin1()));
    }
