...
void QInstallPage::createSystemFiles() {
    ...
    if ( egs_c_utils_ok ) {
        hasCComp  = QString(HAS_C_COMPILER);
        wouldbe_c_util = QString();
        ...
        hasCComp = QString(HAS_NO_C_COMPILER);
        egs_c_obj = QString();
...
void QInstallPage::buildEGSCode( const QString& workDir ) {
    ...
    QStringList args;
    args << "EGS_CONFIG=" + specFile << "EGS_HOME="   + egsHome();
    procStop();
