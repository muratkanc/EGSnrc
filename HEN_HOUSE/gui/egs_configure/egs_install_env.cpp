...
#else
    // User requests the folder on the desktop
    QStringList guiscript;
    QString scriptdummy(gui_script), script, scriptstr;
    guiscript.append("egui");
    guiscript.append("rzgui");
    guiscript.append("viewgui");
    QString lnkdummy(guilnk);
    QString lnkstr;
...
void QInstallPage::setup_static_guis() {
    ...
    QStringList gui;
    QChar s =  QDir::separator();
    QString lnkname, target, syslink;
    gui.append("egs_gui");
    gui.append("egs_inprz");
    gui.append("egs_view");
    for ( QStringList::Iterator it = gui.begin(); it != gui.end(); ++it ) {
