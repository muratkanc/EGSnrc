...
EGS_CompilePage::EGS_CompilePage(QWidget *parent, const char *name,
                                 WFlags f) : EGS_GUI_Widget(parent,name,f) {
    make();
}

EGS_CompilePage::EGS_CompilePage(EGS_ConfigReader *cr, QWidget *parent,
                                 const char *name, WFlags f) : EGS_GUI_Widget(cr,parent,name,f) {
    make();
}
...
void EGS_CompilePage::make() {
    ...
    QVBoxLayout *bgl = new QVBoxLayout(c_option);
    bgl->setSpacing(6);
    bgl->setMargin(11);
    ...
    bgl->addWidget(rb);
    the_target = 0;
    bg_coption->addButton(rb);
    rb = new QRadioButton("noopt",c_option);
    bgl->addWidget(rb);
    bg_coption->addButton(rb);
    rb = new QRadioButton("debug",c_option);
    bgl->addWidget(rb);
    bg_coption->addButton(rb);
    rb = new QRadioButton("clean",c_option);
    bgl->addWidget(rb);
    bg_coption->addButton(rb);
    ...
    vl1->addWidget(gb);
    // C Options
    gb = new QGroupBox("Extra C options",this);
...
void EGS_CompilePage::startCompilation() {
    ...
    QString conf = "EGS_CONFIG=";
    conf += egsConfiguration();
    args <<conf; //c_process->addArgument(conf);
    QString ehome = "EGS_HOME=";
    ehome += egsHome();
...
#endif
    start_c->setEnabled(true);
    stop_c->setEnabled(false);
