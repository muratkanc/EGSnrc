...
EGS_FanoSource::EGS_FanoSource(EGS_Input *input,
    ...
    if(!err) min_theta = tmp_theta/180.0*M_PI;

    err = input->getInput("max theta", tmp_theta);
    if(!err) max_theta = tmp_theta/180.0*M_PI;

    err = input->getInput("min phi", tmp_theta);
    if(!err) min_phi = tmp_theta/180.0*M_PI;
...
void EGS_FanoSource::setUp() {
    ...
    if( !isValid() ) description = "Invalid Fano source";
    else {
