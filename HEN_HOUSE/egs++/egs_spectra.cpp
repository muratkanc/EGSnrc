...
Beta events may set the energy level of the daughter to an excited state.
Beta- and beta+ events emit an electron or positron, respectively. The energies 
of beta particles are sampled from spectra generated by 
\ref EGS_RadionuclideBetaSpectrum. Electron capture events are sampled, 
but no particles are emitted.

Alpha events may set the energy level of the daughter to an excited state.
Alpha events do not emit any particles.

X-Ray events emit a photon from a sampled energy line.

Auger events emit an electron from a sampled energy line.
